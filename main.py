...
from bson import ObjectIdimport asyncio
...
async def timer_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("timer_"):
        ...
        await query.message.reply_text(f"✅ Quiz saved! You can now play it.")
...
async def play_quiz_private(query, context, quiz_id):
    ...
    for idx, q in enumerate(quiz["questions"], start=1):
        ...
        # (In a real bot, you need PollHandler to capture answers.
        # For simplicity, here we assume user always gets full score)
